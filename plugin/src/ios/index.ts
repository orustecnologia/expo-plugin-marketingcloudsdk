...
const withInfo: ConfigPlugin<MarketingCloudSdkPluginProps> = (
  config,
  props
) => {
  return withInfoPlist(config, (config) => {
    ...
    config.modResults.SFMCPiAnalyticsEnabled =
      props.piAnalyticsEnabled ?? false;
    config.modResults.SFMCApplicationControlsBadging =
      props.applicationControlsBadging ?? false;
    config.modResults.SFMCDelayRegistrationUntilContactKeyIsSet =
      props.delayRegistrationUntilContactKeyIsSet ?? false;
    config.modResults.SFMCInboxEnabled = props.inboxEnabled ?? false;
    config.modResults.SFMCUseLegacyPIIdentifier =
      props.useLegacyPIIdentifier ?? false;
...
const withRemoteNotificationsBackgroundMode: ConfigPlugin<
